...
# Talio HRMS - Build APK for mwg.talio.in
# This script builds the Android APK with the new domain

set -e  # Exit on error

echo "🚀 Building Talio HRMS APK for mwg.talio.in"
echo "=============================================="
echo ""
...
if [ -f "android/app/build/outputs/apk/release/app-release.apk" ]; then
    cp android/app/build/outputs/apk/release/app-release.apk "$RELEASE_DIR/talio-hrms-mwg.apk"
    echo -e "${GREEN}✅ APK copied to $RELEASE_DIR/talio-hrms-mwg.apk${NC}"
else
...
if [ -f "android/app/build/outputs/bundle/release/app-release.aab" ]; then
    cp android/app/build/outputs/bundle/release/app-release.aab "$RELEASE_DIR/talio-hrms-mwg.aab"
    echo -e "${GREEN}✅ AAB copied to $RELEASE_DIR/talio-hrms-mwg.aab${NC}"
else
...
# Get APK info
if [ -f "$RELEASE_DIR/talio-hrms-mwg.apk" ]; then
    APK_SIZE=$(du -h "$RELEASE_DIR/talio-hrms-mwg.apk" | cut -f1)
    echo ""
...
# Talio HRMS - Android Release (mwg.talio.in)

## Build Information
- **Package Name**: in.talio.mwg
- **Domain**: mwg.talio.in
- **Version**: 1.0.1 (Build 2)
- **Build Date**: $(date)

## Files Included
1. **talio-hrms-mwg.apk** - Android APK file (for direct installation)
2. **talio-hrms-mwg.aab** - Android App Bundle (for Google Play Store)
...
2. Transfer `talio-hrms-mwg.apk` to your device
3. Open the APK file and follow installation prompts

### Play Store Deployment (AAB)
1. Go to Google Play Console
2. Create a new app or select existing app
3. Upload `talio-hrms-mwg.aab` to the release track
...
```
https://mwg.talio.in/.well-known/assetlinks.json
```
...
- Base URL: https://mwg.talio.in
- Make sure SSL certificate is valid
- Upload assetlinks.json to the domain

## Deployment Checklist

- [ ] Upload assetlinks.json to https://mwg.talio.in/.well-known/assetlinks.json
- [ ] Verify SSL certificate is valid for mwg.talio.in
...
echo "1. Upload assetlinks.json to https://mwg.talio.in/.well-known/assetlinks.json"
echo "2. Install APK on your device: $RELEASE_DIR/talio-hrms-mwg.apk"
